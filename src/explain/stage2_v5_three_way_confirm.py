"""Stage 2 Run B v5 -- confirmation of the evidence-adaptive-graph-driven
3-way candidates, using the EXACT existing v3 lower-order-vs-full 3-way
grouped-CV logic UNCHANGED (literal function imports, no reimplementation,
...
def main():
    ...
    ds = split.ds
    cand = pd.read_csv(f"{R}/stage2_runB_v5_three_way_candidates.csv")
    ...
    print(f"Confirming {len(triples)} v5 3-way candidates using the unchanged v3 lower-order-vs-full test")
    ...
    for k, (a, b, c) in enumerate(triples):
        if a not in ds.name_map or b not in ds.name_map or c not in ds.name_map:
            continue
        cv = lower_vs_full_cv(get_x(a), get_x(b), get_x(c), y_train_arr, folds, n_bins=2)
        if cv is None:
            continue
    ...
    out_path = f"{R}/stage2_runB_v5_three_way_interactions.csv"
    df.to_csv(out_path, index=False)
    print(f"\nSaved {out_path} ({len(df)} confirmed triples, {time.time()-t0:.1f}s)")
