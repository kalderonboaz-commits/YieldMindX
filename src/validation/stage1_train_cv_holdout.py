"""Stage 1 train (in-sample) vs grouped-CV vs holdout generalization check,
original predictors only, for all three models.
"""
...
def compute_train_insample_metrics(fm_train, y_train, feature_cols_original, models):
    ...
    rows = []
    for m in models:
        pred = _fit_predict(m, X, y_train, X, random_state=42)
...
def main():
    ...
    print("=== TRAIN (in-sample) metrics, Stage 1 common set, seed=42 ===")
    train_df = compute_train_insample_metrics(fm_train, y_train, common_set, models)
    print(train_df.round(4).to_string(index=False))

    print("\n=== CV (out-of-fold within train portion) metrics, seed=42 ===")
    cv_df = cross_validate_on_columns(fm_train, y_train, groups_train, common_set, models=models)
    cv_summary = summarize_cv(cv_df)
    print(cv_summary.round(4).to_string())

    print("\n=== HOLDOUT (untouched 20 lots) metrics, seed=42 ===")
    hold_df = evaluate_holdout_on_columns(fm_train, y_train, fm_test, y_test, common_set, models=models)
    ...
    train_df.to_csv(OUTPUTS_DIR / "reports" / "stage1_train_insample_metrics.csv", index=False)
    cv_df.to_csv(OUTPUTS_DIR / "reports" / "stage1_cv_metrics.csv", index=False)
    hold_df.to_csv(OUTPUTS_DIR / "reports" / "stage1_holdout_metrics_seed42.csv", index=False)
    print("\nSaved Stage 1 train/CV/holdout CSVs.")
